...
namespace Miscellaneous.Easing
{
    public static class EasingFunctions
    {
        ...
        public static float Ease(TYPE type, float normalized_time, float start_value, float end_value)
        {
            switch (type)
            {
                ...
                case TYPE.Regular:
                    {
                        var factor = normalized_time;
                        var one_minus_factor = 1.0f - factor;
        ...
        public static Vector3 Ease(TYPE type, float normalized_time, Vector3 start_value, Vector3 end_value)
        {
            ...
            var factor = Ease(type, normalized_time, 0.0f, 1.0f);
            var one_minus_factor = 1.0f - factor;
        ...
        public static TransformValue Ease(TYPE type, float normalized_time, TransformValue start_value, TransformValue end_value)
        {
            float factor = Ease(type, normalized_time, 0.0f, 1.0f);

            TransformValue value = new TransformValue(
                Vector3.Lerp(start_value.LocalPosition, end_value.LocalPosition, factor),
                Quaternion.Lerp(start_value.LocalRotation, end_value.LocalRotation, factor),
                Vector3.Lerp(start_value.LocalScale, end_value.LocalScale, factor)
                );
            return value;
        }
        ...
        public static float FarEaseIn(float time, float start_value, float end_value)
        {
            float ts = time * time;
            float tc = ts * time;
            float calculated_function = (-1.8f * tc * ts + 6.6f * ts * ts + -9.5f * tc + 6.9f * ts + -1.2f * time);
            float one_minus_calculated_function = 1 - calculated_function;
        ...
        public static float EaseInOut(float time, float start_value, float end_value)
        {
            float ts = time * time;
            float tc = ts * time;
            float ease_value = 6 * tc * ts - 15 * ts * ts + 10 * tc;
        ...
        public static float EaseOutElastic(float time, float start_value, float end_value)
        {
            float ts = time * time;
            float tc = ts * time;
            float calculated_function = (17.095f * tc * ts - 55.9325f * ts * ts + 69.38f * tc - 40.19f * ts + 10.6475f * time);
            float one_minus_calculated_function = 1 - calculated_function;
        ...
        public static float EaseOutElasticBig(float time, float start_value, float end_value)
        {
            float ts = time * time;
            float tc = ts * time;
            float calculated_function = (56.0f * tc * ts - 175.0f * ts * ts + 200.0f * tc - 100.0f * ts + 20.0f * time);
            float one_minus_calculated_function = 1 - calculated_function;
        ...
        public static float SmallStretch(float time, float start_value, float end_value)
        {
            float ts = time * time;
            float tc = ts * time;
            float calculated_function = (4.895f * tc * ts + -9.19f * ts * ts + 5.595f * tc + -3.4f * ts + 3.1f * time);
            float one_minus_calculated_function = 1 - calculated_function;
        ...
        public static float OutOverShoot(float time, float start_value, float end_value)
        {
            float ts = time * time;
            float tc = ts * time;
            float calculated_function = (11.2975f * tc * ts + -23.195f * ts * ts + 30.695f * tc + -36.995f * ts + 19.1975f * time);
            float one_minus_calculated_function = 1 - calculated_function;
